...
from keras.layers import Layer, Input, Dense, BatchNormalization, Conv1D, MaxPooling1D, LSTM, TimeDistributed, Reshape, concatenate, Conv2D, MaxPooling2D, Flatten, Multiply
...
dc_path = '/Volumes/1708903/MEx/Data/dc_scaled/min'

results_file = 'dc_act_att3.csv'
...
class WeightedLayer(Layer):
    ...
    def call(self, inputs):
        ...
        # (, 1, 200)
        alpha = tf.nn.l2_normalize(alpha, axis=-1)
        print(K.int_shape(alpha))
        # (, 1, 200)
...
def read(path, _sensor):
    ...
    for subject in [f for f in subjects if not f.startswith('.')]:
        allactivities = {}
...
def split_windows(act_data, dc_data):
    outputs = []
    ...
    while start + _window < end:
        ...
        instances = [act_instances, dc_instances]
        outputs.append(instances)
    return outputs


def extract_features(act_data, dc_data):
    _features = {}
...
def flatten(_data):
    ...
    return dct_act, dc, flatten_labels
...
def _run_(act_train_features, dc_train_features, train_labels, act_test_features, dc_test_features, test_labels):
    ...
    model.fit([act_train_features, dc_train_features], train_labels, verbose=1, batch_size=32, epochs=30, shuffle=True)
    _predict_labels = model.predict([act_test_features, dc_test_features], batch_size=64, verbose=0)
    ...
    results = 'act_dc_att3' + ',' + str(fusion)+',' + str(accuracy)+',' + str(f_score)
    print(results)
...
all_features = extract_features(act_data, dc_data)
...
_act_train_features, _dc_train_features, _train_labels = flatten(train_features)
_act_test_features, _dc_test_features, _test_labels = flatten(test_features)
...
_run_(_act_train_features, _dc_train_features, _train_labels, _act_test_features, _dc_test_features, _test_labels)
