...
results_file = '/home/mex/results_lopo/2m/pm_act_2m_att3.csv'
...
def split_windows(act_data, pm_data):
    outputs = []
    start = act_data[0][0]
    ...
    while start + _window < end:
        ...
        instances = [act_instances, pm_instances]
        outputs.append(instances)
    return outputs


def extract_features(act_data, pm_data):
    _features = {}
    for subject in act_data:
        ...
        for act_activity in act_activities:
            ...
            for item in act_activity_data.keys():
                time_windows.extend(split_windows(act_activity_data[item], pm_activity_data[item]))
...
def flatten(_data):
    ...
    return dct_act, pm, flatten_labels
...
def build_late_fusion():
    ...
    model = Model(inputs=[input_t, input_pm], outputs=z)
    model.summary()
    return model


def _run_(act_train_features, pm_train_features, train_labels, act_test_features, pm_test_features, test_labels):
    ...
    model.fit([act_train_features, pm_train_features], train_labels, verbose=1, batch_size=32, epochs=30, shuffle=True)
    _predict_labels = model.predict([act_test_features, pm_test_features], batch_size=64, verbose=0)
    ...
    results = 'act_pm' + ',' + str(fusion) + ',' + str(i) + ',' + str(accuracy)+',' + str(f_score)
    print(results)
...
all_features = extract_features(act_data, pm_data)
...
_act_train_features, _pm_train_features, _train_labels = flatten(train_features)
_act_test_features, _pm_test_features, _test_labels = flatten(test_features)
...
_run_(_act_train_features, _pm_train_features, _train_labels, _act_test_features, _pm_test_features, _test_labels)
