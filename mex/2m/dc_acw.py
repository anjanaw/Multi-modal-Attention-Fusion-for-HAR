...
results_file = '/home/mex/results_lopo/2m/dc_acw_att3.csv'
...
def pad_features(_features):
    ...
    for subject in _features:
        new_activities = {}
        activities = _features[subject]
        for acw in activities:
            items = activities[acw]
            new_items = []
            for item in items:
                new_item = []
                acw_len = len(item[0])
...
def _run_(acw_train_features, dc_train_features, train_labels, acw_test_features, dc_test_features, test_labels):
    ...
    results = 'acw_dc' + ',' + str(fusion)+',' + str(i) + ',' + str(accuracy)+',' + str(f_score)
    print(results)
