...
results_file = '/home/mex/results_lopo/3m/act_dc_pm_matt_final_conf.csv'
...
def split_windows(act_data, dc_data, pm_data):
    outputs = []
    ...
    while start + _window < end:
        ...
        instances = [act_instances, dc_instances, pm_instances]
        outputs.append(instances)
    return outputs


def extract_features(act_data, dc_data, pm_data):
    _features = {}
...
def flatten(_data):
    ...
    return dct_act, dc, pm, flatten_labels
...
def build_late_fusion():
    ...
    model = Model(inputs=[input_t, input_dc, input_pm], outputs=c)
    model.summary()
    return model


def _run_(act_train_features, dc_train_features, pm_train_features, train_labels, act_test_features, dc_test_features,
    ...
    model.fit([act_train_features, dc_train_features, pm_train_features], train_labels, verbose=1, batch_size=32, epochs=30, shuffle=True)
    _predict_labels = model.predict([act_test_features, dc_test_features, pm_test_features], batch_size=64, verbose=0)
    ...
    results = 'act_dc_pm' + ',' + str(fusion)+',' + str(i)+',' + str(accuracy)+',' + str(f_score)
    print(results)
    write_data(results_file, str(results))

    _test_labels = pd.Series(test_labels.argmax(axis=1), name='Actual')
...
all_features = extract_features(_act_data, _dc_data, _pm_data)
...
_act_train_features, _dc_train_features, _pm_train_features, _train_labels = flatten(train_features)
_act_test_features, _dc_test_features, _pm_test_features, _test_labels = flatten(test_features)
...
_run_(_act_train_features, _dc_train_features, _pm_train_features, _train_labels, _act_test_features, _dc_test_features,
