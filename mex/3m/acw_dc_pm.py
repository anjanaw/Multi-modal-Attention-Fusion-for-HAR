...
results_file = '/home/mex/results_lopo/3m/acw_dc_pm_matt2.csv'
...
def split_windows(acw_data, dc_data, pm_data):
    outputs = []
    ...
    while start + _window < end:
        ...
        instances = [acw_instances, dc_instances, pm_instances]
        outputs.append(instances)
    return outputs


def extract_features(acw_data, dc_data, pm_data):
    _features = {}
    for subject in acw_data:
        ...
        for acw_activity in acw_activities:
            ...
            dc_activity_data = dc_data[subject][acw_activity]
            pm_activity_data = pm_data[subject][acw_activity]
            for item in acw_activity_data.keys():
                time_windows.extend(split_windows(acw_activity_data[item], dc_activity_data[item],
...
def flatten(_data):
    ...
    return dct_acw, dc, pm, flatten_labels
...
def build_late_fusion():
    ...
    w = WeightedLayer(300)([x, y, z])
    a = AttentionLayer(300)([x, y, z])
    c = concatenate([w, a])
    c = Dense(len(activity_list), activation='softmax')(c)

    model = Model(inputs=[input_w, input_dc, input_pm], outputs=c)
    model.summary()
    return model


def _run_(acw_train_features, dc_train_features, pm_train_features, train_labels, acw_test_features, dc_test_features,
    ...
    model.fit([acw_train_features, dc_train_features, pm_train_features], train_labels, verbose=1, batch_size=32, epochs=30, shuffle=True)
    _predict_labels = model.predict([acw_test_features, dc_test_features, pm_test_features], batch_size=64, verbose=0)
    ...
    results = 'acw_dc_pm' + ',' + str(fusion)+',' + str(i)+',' + str(accuracy)+',' + str(f_score)
    print(results)
...
all_features = extract_features(_acw_data, _dc_data, _pm_data)
...
_acw_train_features, _dc_train_features, _pm_train_features, _train_labels = flatten(train_features)
_acw_test_features, _dc_test_features, _pm_test_features, _test_labels = flatten(test_features)
...
_run_(_acw_train_features, _dc_train_features, _pm_train_features, _train_labels, _acw_test_features, _dc_test_features,
      _pm_test_features, _test_labels)
